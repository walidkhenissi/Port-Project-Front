...
// MatPaginator Output
  ...
  public generateReportState() {
    ...
    if (this.commissionaryPayments) {
      ...
      options.isMerchant = this.selectedProducer ? this.selectedProducer.isMerchant : false;
      options.isShipOwner = this.selectedProducer ? this.selectedProducer.isShipOwner : false;
    } else {
  ...
  public generateAccountReportState() {
    const options = {
      dateRule: this.rule,
      startDate: this.formatDate(this.date1) || new Date(),
      endDate: this.rule == "equals" ? null : (this.formatDate(this.date2) || new Date()),
