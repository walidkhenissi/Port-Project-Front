...
// MatPaginator Output
  ...
  displayedColumns: string[] = ['date', 'producerName', 'saleName', 'paymentName', 'value'];
  ...
  public filterEvent(attr: string, type: string, entity: string | null = null, label: string | null = null) {
    this.dialog.open(FilterDialogComponent, {
      data: {
        ...
        forcedEnumFilter: entity?.toUpperCase() == 'PAYMENT' ? {isCommissionnaryPayment: true} : null,
        forcedEnumFindMethod: entity?.toUpperCase() == 'PRODUCER' ? {
          service: this.shipownerService,
          method: 'findProducer'
        } : null,
  ...
  public generateReportState() {
    const options = {
      ...
      isMerchant: this.selectedProducer ? this.selectedProducer.isMerchant : false,
      isShipOwner: this.selectedProducer ? this.selectedProducer.isShipOwner : false
    };
